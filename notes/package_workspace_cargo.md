...
Per quanto riguarda i workspace, non ho molto da aggiungere: come modello mentale, il concetto mi sembra molto simile a quello di una *Solution* nel mondo .NET/C#.

Un workspace raggruppa infatti uno o più package che vengono gestiti insieme, così come una Solution raggruppa uno o più progetti.

Il paragone mi sembra ancora più intuitivo con il nuovo formato `.slnx`, che utilizza un formato XML facilmente leggibile e modificabile direttamente. In questo senso, il `Cargo.toml` alla radice del workspace svolge un ruolo simile al file di Solution, mentre i `Cargo.toml` dei singoli package possono essere paragonati, almeno concettualmente, ai file `.csproj` dei singoli progetti.

L'analogia è particolarmente vicina nel caso di un **virtual workspace**, cioè quando il `Cargo.toml` alla radice contiene la sezione `[workspace]` ma non definisce esso stesso un `[package]`.
...
Un'altra somiglianza riguarda le dipendenze tra i progetti/package.

Nel mondo .NET, un progetto può dipendere da un altro progetto della stessa Solution tramite un `ProjectReference` dichiarato nel `.csproj` del progetto dipendente.

In Rust si può fare qualcosa di concettualmente simile dichiarando una dipendenza tramite `path` nel `Cargo.toml` del package dipendente:
...
È però importante distinguere due concetti: **appartenere allo stesso workspace** e **dipendere da un altro package** non sono la stessa cosa.

La sezione `[workspace]` stabilisce quali package vengono gestiti insieme dal workspace, mentre `[dependencies]` stabilisce quali package vengono effettivamente utilizzati da un altro package.

Quindi due package possono appartenere allo stesso workspace senza dipendere l'uno dall'altro.

Il workspace offre inoltre alcune funzionalità comuni a tutti i suoi membri. Per esempio, i package del workspace condividono lo stesso `Cargo.lock` e, normalmente, la stessa directory `target/`. È anche possibile centralizzare alcune configurazioni, come metadata, dipendenze e lint, tramite sezioni come `[workspace.package]`, `[workspace.dependencies]` e `[workspace.lints]`.
...
Solution              ≈    Workspace
.slnx                  ≈    Cargo.toml con [workspace]
Project (.csproj)      ≈    Package (Cargo.toml)
ProjectReference       ≈    dependency con path
```

mi sembra un buon modello mentale, purché venga considerato come un'analogia e non come una corrispondenza perfetta tra i due sistemi.
